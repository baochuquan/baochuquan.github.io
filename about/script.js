...
var M = {
	...
	c: null,	// the existed canvas in index.html
	ctx: null,
...
*/
	draw: function() {
		...
		for (var i = 0; i < M.COLUMNS; i++) {
			...
			if (M.codes[i][0].canvas) {
				...
				M.ctx.drawImage(c, x, y, M.settings.COL_WIDTH, height);	//add canvas to main canvas

				if ((M.codes[i][0].position.y - height) < M.HEIGHT){
					M.codes[i][0].position.y += velocity;				//update the y (top of sub canvas)
				} else {
	...
	createCode: function() {		// call itself M.COLUMNS times when used
		...
		var column = M.assignColumn();	//get a random column
		...
		if (column) {
			...
			for (var i = 1; i <= codeLength; i++) {		//get the letters
				var newLetter = M.randomFromInterval(0, (lettersLength - 1));
				M.codes[column][i] = M.letters[newLetter];	//codes[columns][i]
			}
	...
	createCanvii: function(i) {		//the i th column
		...
		M.codes[i][0].canvas = newCanv;			//define the canvas
	},
/*
...
*/
	...
	assignColumn: function() {
		var randomColumn = M.randomFromInterval(0, (M.COLUMNS - 1));	//get a random num 
...
*/
	...
	snapshot: function() {
		M.createLines(M.ctx);
		window.open(M.c.toDataURL());	//convert canvas to s static picture
	}
